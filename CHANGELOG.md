...
### Changed
- Package renamed to `@peerbits/fhir-observation-generator` (npm rejected the unscoped name as too similar to the existing `fhirclient` package).
...
- `readingsToBundle` transaction Bundle compiler composable with `@peerbits/fhir-client`.
...
- Code examples for basic conversion and batch transaction bundle execution with `@peerbits/fhir-client`.
